...
import { colorDistance } from "../lib/beads";
...
describe("CIEDE2000 本地颜色匹配", () => {
  it("通过全部 34 组公开参考数据", () => {
    for (const [l1, a1, b1, l2, a2, b2, expected] of REFERENCE) {
      const left: [number, number, number] = [l1, a1, b1],
        right: [number, number, number] = [l2, a2, b2];
      expect(colorDistance(left, right)).toBeCloseTo(expected, 4);
      expect(colorDistance(left, right)).toBeCloseTo(
        colorDistance(right, left),
        10,
      );
    }
    expect(colorDistance([50, 10, -20], [50, 10, -20])).toBe(0);
  });
});
