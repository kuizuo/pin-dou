...
const questions = [
  [
    "没有 AI 密钥也能用吗？",
    "可以。AI 处理默认可使用 Cloudflare；也可以在设置中选择 GPT Image 或 Gemini，并填写自己的 API Key。",
  ],
  [
    "图片和密钥会保存吗？",
    "裁切后的图片只用于当次 AI 请求。OpenAI 和 Gemini 密钥保存在当前浏览器，方便下次直接使用，但不会写入作品或备份。",
  ],
  ["支持哪些拼豆品牌？", "目前只支持 MARD，使用固定来源的完整 291 色。"],
  [
    "作品保存在哪里？",
    "原图、设置和当前图纸都保存在当前浏览器。建议定期使用“备份全部”，换设备后可以恢复。",
  ],
  [
    "格数和颜色该怎么选？",
